...
/**
 * Generate a V0 DynamicTnid where both the V0 and its encrypted V1 form
 * contain no blocklisted words.
 *
 * Like {@link newV0FilteredForEncryption} but accepts a runtime name string.
 *
 * @throws {FilterError} If maximum iterations exceeded.
 */
export async function newDynamicV0FilteredForEncryption(
  name: string,
  blocklist: Blocklist,
  key: EncryptionKey,
): Promise<DynamicTnid> {
  ...
  for (let i = 0; i < maxIterations; i++) {
    const random = randomBigInt(V0_RANDOM_BITS);
    const v0 = DynamicTnid.newV0WithParts(name, timestamp, random);
    const v0Data = dataString(v0);
